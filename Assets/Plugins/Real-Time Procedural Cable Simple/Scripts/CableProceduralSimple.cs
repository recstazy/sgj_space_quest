...
public class CableProceduralSimple : MonoBehaviour
{
	...
	[SerializeField] float _threshold = 0.2f;
	...
	void Start () 
	{
		...
		_endPointPosition = endPointTransform.position;
        SetEndPosition();
    }
	...
	void Update () 
	{
        if (_endPointPosition != endPointTransform.position)
        {
            SetEndPosition();
        }
        Draw();
	...
	void Draw()
	{
		...
		if(Vector3.Distance(transform.position, _endPointPosition)< 0.2f)
		{
			line.enabled = false;
			return;
		}
